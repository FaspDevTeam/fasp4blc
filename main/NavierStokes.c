...
int main(int argc, const char* argv[])
{
    ...
    // --------------------------- //
        ...
        // get Auu block
        fasp_dcsr_getblk(&Acsr, u_idx.val, u_idx.val, u_idx.row, u_idx.row,
                         A.blocks[0]);
        // get Aup block
        fasp_dcsr_getblk(&Acsr, u_idx.val, p_idx.val, u_idx.row, p_idx.row,
                         A.blocks[1]);
        // get Apu block
        fasp_dcsr_getblk(&Acsr, p_idx.val, u_idx.val, p_idx.row, u_idx.row,
                         A.blocks[2]);
        // get App block
        fasp_dcsr_getblk(&Acsr, p_idx.val, p_idx.val, p_idx.row, p_idx.row,
                         A.blocks[3]);
    ...
    if (itsolver_type == 0) {
        ...
        for (i = 0; i < nrow; i++) b_test.val[i] = b.val[i];
        ...
        if (problem_num > 100 && problem_num < 100 ){
            flag = fasp_solver_dblc_krylov_navier_stokes_with_pressure_mass(&A, &b, &uh,
        &itparam, &amgparam, &iluparam, &schparam, &Mp);
        }
         */
        /*
        else if ( problem_num > 200 && problem_num < 300 ) {
            flag =
        fasp_solver_dblc_krylov_navier_stokes_schur_complement_with_pressure_mass(&A,
        &b, &uh, &itparam, &amgparam, &iluparam, &schparam, &Mp);
        }
         */
        // else {
        flag = fasp_solver_dblc_krylov_navier_stokes(&A, &b, &uh, &itparam, &amgparam,
                                                     &iluparam, &schparam);
        //}
    }
